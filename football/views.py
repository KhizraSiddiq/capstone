...
def index(request):
    # Assuming response_standings and response_teams are defined elsewhere in your code
    standings_data = response_standings.json().get("standings", [])
    
    # Check if there is at least one team in the standings
    if standings_data and isinstance(standings_data[0], dict):
        table = standings_data[0].get("table", [])

        # Check if the table is not empty before accessing the first team
        if table and isinstance(table[0], dict):
            first_team_name = table[0]["team"]["name"]
        else:
            first_team_name = "No teams available"
    else:
        first_team_name = "No standings data available"

    return render(request, "football/index.html", {
        "teams": response_teams.json().get("teams", []),
        "current_matchday": str(current_matchday),
        "first": first_team_name
    })
...
def forum(request):
    ...
    for team in teams:
        team["count"] = Thread.objects.filter(forum=team["tla"]).count()
...
def newPost(request, tla="general"):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            newthread.content = html2text.html2text(newthread.content) 
...
def thread(request, slug):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            newReply.content = html2text.html2text(newReply.content)
